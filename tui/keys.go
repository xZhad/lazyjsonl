...
var keys = keyMap{
	...
	Page:   key.NewBinding(key.WithKeys("h", "l", "left", "right"), key.WithHelp("h/l", "page")),
	Ends:   key.NewBinding(key.WithKeys("g", "G"), key.WithHelp("g/G", "first/last")),
	Column: key.NewBinding(key.WithKeys("H", "L", "alt+left", "alt+right"), key.WithHelp("H/L", "column")),
...
func (m *Model) shortKeys() []key.Binding {
	...
	if len(m.drillPath) > 0 {
		return []key.Binding{keys.Back, keys.Dive, keys.Sort, keys.Filter, keys.Cols, keys.Help, keys.Quit}
	}
	return []key.Binding{keys.Move, keys.Filter, keys.Enter, keys.Sort, keys.Dive, keys.Cols, keys.Tab, keys.Help, keys.Quit}
}

// fullGroups returns the columns for the full help overlay.
func fullGroups() [][]key.Binding {
	return [][]key.Binding{
		{keys.Move, keys.Page, keys.Ends, keys.Column, keys.File, keys.Tab},
		{keys.Enter, keys.Sort, keys.Dive, keys.Back, keys.Cols, keys.Filter, keys.Esc},
		{keys.Delete, keys.Export, keys.Yank, keys.Reload, keys.Mouse, keys.Help, keys.Quit},
	}
}
